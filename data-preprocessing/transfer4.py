...
class YOLOMosaicGenerator:
    ...
    def _convert_label_to_mosaic(self, labels, img_h, img_w, position, mosaic_h, mosaic_w):
        ...
        参数:
            ...
            position: 图像在mosaic中的位置 (row, col)
                     (0,0)=左上角, (0,1), (0,2), (0,3)=右上角
                     (1,0), (1,1), (1,2), (1,3)
                     (2,0), (2,1), (2,2), (2,3)
                     (3,0)=左下角, (3,1), (3,2), (3,3)=右下角
    ...
    def create_mosaic(self, image_paths):
        ...
        if len(image_paths) != 16:
            raise ValueError(f"需要16张图像,但提供了{len(image_paths)}张")
    ...
    def generate_dataset(self):
        """
        生成mosaic数据集
        按顺序每16张图拼接成1张mosaic,不重复使用
        """
        total_images = len(self.image_files)
        num_mosaics = total_images // 16
        remaining = total_images % 16
        
        print("\n" + "=" * 60)
        print(f"原始数据集: {total_images} 张图像")
        print(f"将生成: {num_mosaics} 张 mosaic 图像 (4x4拼接)")
        print(f"每张mosaic使用: 16 张原始图像")
        if remaining > 0:
            print(f"⚠️  剩余: {remaining} 张图像无法凑成16张,将被跳过")
        print("\n拼接布局:")
        print("  [图1]  [图2]  [图3]  [图4]")
        print("  [图5]  [图6]  [图7]  [图8]")
        print("  [图9]  [图10] [图11] [图12]")
        print("  [图13] [图14] [图15] [图16]")
        print("=" * 60)
        
        # 确认是否继续
        if num_mosaics == 0:
            print("\n❌ 错误: 图像数量少于16张,无法生成mosaic!")
            return
        ...
        for i in tqdm(range(num_mosaics), desc="生成Mosaic数据集"):
            # 按顺序取16张图
            start_idx = i * 16
            selected_images = self.image_files[start_idx:start_idx + 16]
        ...
        print(f"\n验证:")
        print(f"  原始图像使用: {success_count * 16} / {total_images} 张")
        print(f"  利用率: {success_count * 16 / total_images * 100:.1f}%")
        print("=" * 60)
...
if __name__ == "__main__":
    ...
    - 布局: [图1] [图2] [图3] [图4]
            ...
            [图13][图14][图15][图16]
    - 不重复使用任何图片
    - 如果图片数量不是16的倍数,剩余的会被跳过
    """
    
    # ========== 配置参数 ==========
    
    # 原始数据集路径
    SOURCE_IMAGES_DIR = "data/train/images"  # 修改为你的路径
    SOURCE_LABELS_DIR = "data/train/labels"  # 修改为你的路径
    
    # 输出路径
    OUTPUT_IMAGES_DIR = "dataset_mosaic4/train/images"
    OUTPUT_LABELS_DIR = "dataset_mosaic4/train/labels"
    ...
    print("3. 原始图片顺序: img1~img16 → mosaic_000000.jpg")
    print("                 img17~img32 → mosaic_000001.jpg")
    print("                 ...")